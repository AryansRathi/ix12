...
Example `.env` configuration (see full list of variables in [Environment variables](#administration-api-runtime)):
```.env
...
The services can be configured using various environment variables.

## Configurable via `.env` file
All of the following environment variables can be defined in the `.env` file. They are referenced in the respective Compose files and therefore directly affect the deployment.

### General
| Variable              | Description                         | Default   | Notes                                                                                                                   |
| --------------------- |------------------------------------ | --------- |------------------------------------------------------------------------------------------------------------------------ |
| `IX_VERSION`          | Intrexx version to be built and run | `latest`  | See [download server](https://download.intrexx.com/intrexx/rolling/steady/) for available versions starting from 12.0.0 |
...
The built Intrexx server image provides additional runtime environment variables. These are not included in the `.env` file, so if you want to use them, you'll need to modify the relevant Compose files.

### Tomcat headers
| Variable                                          | Default standalone | Default distributed |
|-------------------------------------------------- | ------------------ |-------------------- |
| `TOMCAT_SEC_HEADER_XUSER`                         | deny               | deny                |
| `TOMCAT_SEC_HEADER_XDOMAIN`                       | deny               | deny                |
| `TOMCAT_SEC_HEADER_XKRBTICKET`                    | deny               | deny                |
| `TOMCAT_SEC_HEADER_XACCOUNTNAME`                  | deny               | allow               |
| `TOMCAT_SEC_HEADER_XFORWARDEDFOR`                 | deny               | allow               |
| `TOMCAT_SEC_HEADER_FORWARDED`                     | deny               | allow               |
| `TOMCAT_SEC_HEADER_XREALIP`                       | deny               | allow               |
| `TOMCAT_SEC_HEADER_XFORWARDEDHOST`                | deny               | allow               |
| `TOMCAT_SEC_HEADER_XFORWARDEDPROTO`               | deny               | allow               |
| `TOMCAT_SEC_HEADER_XORIGINALURL`                  | deny               | allow               |
| `TOMCAT_SEC_HEADER_RECEIVEONNONLOOPBACKINTERFACE` | false              | true                |

### MISC
| Variable             | Description                                                                                | Default |
| -------------------- | ------------------------------------------------------------------------------------------ | ------- |
| `SOLR_SASL_DISABLED` | If Solr is run without SASL, this parameter adjusts the Intrexx search client accordingly. | true    |
...
> [!TIP]
> Before you migrate your Intrexx deployment to the new Docker Compose example described in this repository, update PostgreSQL in your old deployment to version 18+.
...
* The project name has changed from `intrexx-in-docker` to `ix12-intrexx-in-docker`
...
There are different ways to migrate the existing Docker volumes from the previous example deployment. Two of them are described below.

### Option 1 – Reuse the old volumes

The quickest way is to continue using the existing old volumes by explicitly referencing them by name and defining them as external. This has been prepared in `compose.migration.yaml`.
...
#### Create the new volumes and copy the data
Make sure that the variables in the following example are initialized with the correct values for your environment.

> [!NOTE]
>The Docker Compose project name usually matches the folder name. In some cases, it may differ from the folder name. For example, when the `COMPOSE_PROJECT_NAME` environment variable is used.
